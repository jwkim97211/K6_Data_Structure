...
public class homework_2 {
	...
	static void ShowPolynomial(String str, Polynomial3[] x, int count) {
		...
		// count가 -1이면 다항식 x의 length로 계산하고 양수이면 count가 다항식 항의 숫자이다
		...
		for (int i = 0; i < n; i++)
			if (i != n - 1)
				System.out.printf("%.2f x ** %d + ", x[i].coef, x[i].exp);
			else
	...
	static int AddPolynomial(Polynomial3[] x, Polynomial3[] y, Polynomial3[] z) {
		...
		while (p < x.length && q < y.length) {
			if (x[p].exp == y[q].exp) {
				...
				z[r++] = new Polynomial3(y[q].coef, y[q].exp);
				q++;
				terms++;
			}
		}
	...
	static int MultiplyPolynomial(Polynomial3[] x, Polynomial3[] y, Polynomial3[] z) {
		...
		for (p = 0; p < x.length; p++) {
			for (q = 0; q < y.length; q++) {
				Polynomial3 newTerm = new Polynomial3(x[p].coef * y[q].coef,x[p].exp + y[q].exp);
	...
	static double EvaluatePolynomial(Polynomial3[] z, int zTerms, int value) {
		...
		for (int j = 0; j < zTerms; j++)
			result += z[j].coef * Math.pow(value, z[j].exp);
		return result;
	}

	public static void main(String[] args) {
		Polynomial3[] x = { new Polynomial3(1.5, 3),
		...
		Polynomial3[] y = { new Polynomial3(1.5, 1), 
